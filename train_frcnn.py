...
optimizer = sgd#Adam(lr=1e-3)
optimizer_classifier = sgd#Adam(lr=1e-3)
...
def validate():
	...
	while True:
		...
		loss_rpn_val = model_rpn.evaluate(X_val, Y_val, batch_size = 256, verbose = 0)
		
		P_rpn_val = model_rpn.predict_on_batch(X_val)
		#raw_input('Stop')
		R = roi_helpers.rpn_to_roi(P_rpn_val[0], P_rpn_val[1], C, K.image_dim_ordering(), use_regr=True, overlap_thresh=0.7, max_boxes=300)
		# note: calc_iou converts from (x1,y1,x2,y2) to (x,y,w,h) format
		X2, Y1, Y2, IouS = roi_helpers.calc_iou(R, img_data_val, C, class_mapping)
		...
		loss_class_val = model_classifier.evaluate([X_val, X2[:, sel_samples, :]], [Y1[:, sel_samples, :], Y2[:, sel_samples, :]])
		#raw_input('Again')
		val_losses[val_iter_num, 0] = loss_rpn_val[1]
		val_losses[val_iter_num, 1] = loss_rpn_val[2]

		val_losses[val_iter_num, 2] = loss_class_val[1]
		val_losses[val_iter_num, 3] = loss_class_val[2]
		val_losses[val_iter_num, 4] = loss_class_val[3]
		val_iter_num += 1
		...
		if val_iter_num == epoch_length:
			...
			if curr_loss < best_loss:
				...
				best_loss = curr_loss
				model_all.save_weights('New ResNet50 Model')
